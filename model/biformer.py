...
from model.utils import Attention, AttentionLePE, DWConv
from model.utils import BiLevelRoutingAttention
    ...
    def forward(self, x):
        ...
        if self.pre_norm:
            if self.use_layer_scale:
                ...
                x = x + self.drop_path(self.gamma1 * attention)  # (N, H, W, C)
                x = x + self.drop_path(self.gamma2 * self.mlp(self.norm2(x)))  # (N, H, W, C)
            else:
                attention = self.attn(self.norm1(x))
                x = x + self.drop_path(attention)  # (N, H, W, C)
                x = x + self.drop_path(self.mlp(self.norm2(x)))  # (N, H, W, C)
        else:  # https://kexue.fm/archives/9009
            if self.use_layer_scale:
                attention = self.attn(x)
                x = self.norm1(x + self.drop_path(self.gamma1 * attention))  # (N, H, W, C)
                x = self.norm2(x + self.drop_path(self.gamma2 * self.mlp(x)))  # (N, H, W, C)
            else:
                attention = self.attn(x)
                x = self.norm1(x + self.drop_path(attention))  # (N, H, W, C)
...
class BiFormer(nn.Module):
    def __init__(self,
                 depth=[3, 4, 8, 3], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        for i in range(len(embed_dim) - 1):
            ...
            if (pe is not None) and i + 1 in pe_stages:
                downsample_layer.append(get_pe_layer())
            if use_checkpoint_stages:
