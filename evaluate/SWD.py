...
def swd(image1, image2,
        ...
        n_repeat_projection=128, proj_per_repeat=4,pyramid_batchsize=128):
    ...
    with torch.no_grad():
        ...
        return torch.mean(result).detach().cpu().numpy().item()


def get_SWD(images1, images2):
    images1 = torch.as_tensor(images1).permute(0, 3, 1, 2).float()
    images2 = torch.as_tensor(images2).permute(0, 3, 1, 2).float()
    return swd(images1, images2)
